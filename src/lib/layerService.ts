  ...
  for (const row of data as StoredLayer[]) {
    try {
      ...
      if (downloadError || !fileData) {
        console.error(`Error downloading file for ${row.name}:`, downloadError)
        continue
      }
      ...
      console.error(`Failed to parse file for ${row.name}:`, err)
    }
  }
  return layers
}
...
export async function uploadLayer(
  ...
  const { error: insertError } = await supabase.from('kmz_layers').insert({
    name,
    color,
    visible: true,
    locked,
